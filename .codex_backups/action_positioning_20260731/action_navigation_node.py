...
class ActionNavigationNode(Node):
    ...
    def publish_ready(self, ready, reason):
        ...
        if self.latest_scene_time > 0.0:
            scene_age = time.monotonic() - self.latest_scene_time
    ...
    def handle_active_observe(self):
        ...
        # 必须等机器人到达后的新 scene。
        has_fresh_scene = (
            self.latest_scene_time > self.arrived_scene_time
        )
        ...
        if target_is_servo_usable(
            ...
            self.publish_ready(
                False,
                "target_servo_usable_after_active_observe",
            )
            self.set_phase(Phase.VISUAL_SERVO)
            return
    ...
    def handle_visual_servo(self):
        ...
        if not target or not target.get("centroid_uv"):
            self.visual_servo.stop()
            self.publish_ready(False, "visual_servo_target_missing_centroid")
            self.set_phase(Phase.ACTIVE_OBSERVE)
            return

        aligned = self.visual_servo.step_from_target(target)
    ...
    def handle_ready_for_grasp(self):
        self.navigator.stop()
        self.publish_ready(True, "target_detected_and_base_aligned")

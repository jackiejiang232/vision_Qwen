...
def validate_dino_query_output(raw_text, instruction_text=""):
    ...
    if instruction_text.count("抓取") >= 3 and len(result.tasks) < 3:
        raise ValueError("Qwen DINO query 没有解析出全部任务")
    result.grounding_prompt = (
        " . ".join(parts) + " ."
    )
...
def build_task_queue_from_query(
    ...
    for task in sorted(
        ...
        target_label = (
            target_detection.get("corrected_label")
            or target_detection.get("label")
            if target_detection
            else None
        )
        ...
        task_queue.append(
            {
                ...
                "target": {
                    ...
                    "box_xyxy": (
                        target_detection.get("box_xyxy")
                        if target_detection
                        else None
                    ),
                    "centroid_uv": (
                        target_detection.get("centroid_uv")
                        if target_detection
                        else None
                    ),
                    "mask_area": (
                        target_detection.get("mask_area")
                        if target_detection
                        else None
                    ),
                    "pose_world": (
